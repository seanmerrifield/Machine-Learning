...
%Build y matrix with i rows of examples and k columns of classifications
...
for k = 1:num_labels
    J_labels(k) = 1/m*sum((-y_matrix(:,k).*log(h(:,k)) - (1 - y_matrix(:,k)).*log(1-h(:,k))));
...
Theta1_reg = Theta1;
Theta1_reg(:,1) = [];   %remove first column (bias terms)

Theta2_reg = Theta2;
Theta2_reg(:,1) = [];   %remove first column (bias terms)

Jreg = lambda/(2*m)*(sum(sum(Theta1_reg.^2)) + sum(sum(Theta2_reg.^2)));
